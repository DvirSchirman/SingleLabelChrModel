...
def saveStrainForkPlots(strainID,isrestraint, distToRestraint, modelsPath):
    ...
    allPath = '241023_forkPlots_dropped_inputs_randomModel/'
    allPath = '%s/%s' % (outFolder, allPath)
    if not os.path.isdir(allPath):
        os.mkdir(allPath)
    forkPlotsPath = '%ssimulatedForkPlots_%s/' % (allPath, modelsName)
    radialPlotsPath = '%ssimulatedRadialPlots_%s/' % (allPath, modelsName)
    doghnutPlotsPath = '%ssimulatedDoughnutPlots_%s/' % (allPath, modelsName)
    ...
    # list_models = os.listdir(modelsPath)
    ...
    for i,s in enumerate(np.transpose(strainCoordinatesR)):
        hr = np.histogram(s[~np.isnan(s)],rBins,density=True)[0]
        r1 = rBins[:-1]
        r2 = rBins[1:]
    ...
    if isrestraint:
        ...
        savemat(radialPlotsPath + 'restrainedStrains/' + 'simRadialPLot_M%02d.mat' % strainID, {'heatMap': heatMapRdf.to_numpy()}  )
    else:
        ...
        savemat(radialPlotsPath + 'nonRestrainedStrains/' + 'simRadialPLot_M%02d.mat' % strainID, {'heatMap': heatMapRdf.to_numpy()}  )
    ...
    if isrestraint:
        ...
        savemat(doghnutPlotsPath + 'restrainedStrains/' + 'simYZPLot_M%02d.mat' % strainID, {'heatMap': heatMapDdf.to_numpy()}  )
    else:
        ...
        savemat(doghnutPlotsPath + 'nonRestrainedStrains/' + 'simYZPLot_M%02d.mat' % strainID, {'heatMap': heatMapDdf.to_numpy()}  )
...
lib_tbl = lib_tbl.iloc[inds1,:]
isInRestraints = np.ones((len(lib_tbl)))
if os.path.isfile(modelsPath + '/dropped_loci.txt'):
    dropped_loci = np.loadtxt(modelsPath + '/dropped_loci.txt', dtype=int)
    isInRestraints[list(map(lambda x: np.argwhere(lib_tbl.MG1655_ref_genome_location==x)[0][0], dropped_loci))] = 0
...
# saveStrainForkPlots(3,0, 0, modelsPath)
