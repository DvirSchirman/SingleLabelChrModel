...
def tether_particles_radial(sim_object, particles, *, pbc=False, k=30, positions="current", name="Tethers"):
    ...
    if pbc:
        energy = "kr * periodicdistance(r, 0, 0, r0, 0, 0)^2 + kz * periodicdistance(0, 0, z, 0, 0, z0)^2; r=sqrt(x^2+y^2)"
    else:
        energy = "kr * (r - r0)^2 + kz * (z - z0)^2; r=sqrt(x^2+y^2)"
    ...
    force.addGlobalParameter("kr", kr * sim_object.kT / nm2)
    ...
    # adding all the particles on which force acts
    if len(particles)==1:
        force.addParticle(particles[0], positions)
        if sim_object.verbose == True:
            print("particle %d tethered! " % particles[0])
    else:
